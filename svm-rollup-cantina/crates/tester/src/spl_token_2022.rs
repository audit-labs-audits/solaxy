...
pub(crate) async fn mint_tokens(
    ...
    let token_account_data = unpack_token_account_data(&ata_acc_info.data)
...
pub(crate) async fn transfer_tokens(
    ...
    let sender_token_account_data = unpack_token_account_data(&sender_ata_acc_info.data)
    ...
    let receiver_token_account_data = unpack_token_account_data(&receiver_ata_acc_info.data)
    ...
    let sender_acc_info = client
        ...
        .await
        .map_err(|e| anyhow!("Failed to retrieve account {sender_ata_pubkey}: {e}"))?;
    let sender_token_account_data = unpack_token_account_data(&sender_acc_info.data)
        .map_err(|e| anyhow!("Failed to unpack sender's token account data: {e}"))?;
    debug!("Sender's token account data: {sender_token_account_data:?}");
    ...
    let receiver_acc_info = client
        ...
        .await
        .map_err(|e| anyhow!("Failed to retrieve account {receiver_ata_pubkey}: {e}"))?;
    let receiver_token_account_data = unpack_token_account_data(&receiver_acc_info.data)
        .map_err(|e| anyhow!("Failed to unpack receiver's token account data: {e}"))?;
    debug!("Receiver's token account data: {receiver_token_account_data:?}");
...
pub(crate) async fn burn_tokens(
    ...
    let target_token_account_data = unpack_token_account_data(&target_acc_info.data)
        .map_err(|e| anyhow!("Failed to unpack token account data: {e}"))?;
...
pub fn unpack_token_account_data(account_data: &[u8]) -> anyhow::Result<TokenAccount> {
    ...
    let token_account = TokenAccount::unpack(trimmed_data)
        .map_err(|e| anyhow!("Failed to unpack token account: {e}"))?;
    Ok(token_account)
}

pub(crate) async fn spl_token_2022_scenario(
    client: &TestClient,
    payer: &Keypair,
) -> anyhow::Result<()> {
    debug!("SPL-TOKEN-2022 program scenario");
