...
use crate::{
    da::mock::{new_da_service, new_verifier, DaService, DaSpec},
    zk::{get_outer_vm, InnerCryptoSpec, InnerZkvm, InnerZkvmHost, OuterCodeCommitment, OuterZkvm},
};

/// Svm Rollup Blueprint
pub struct MockSvmRollup<M> {
    genesis_paths: GenesisPaths,
    phantom: std::marker::PhantomData<M>,
}

impl<M> MockSvmRollup<M> {
...
impl Default for MockSvmRollup<Native> {
    fn default() -> Self {
        Self::new(GenesisPaths::from_dir("../test-data/genesis/demo/mock"))
    }
}

type SvmRollupSpec<M> = SolanaSpec<DaSpec, InnerZkvm, OuterZkvm, InnerCryptoSpec, M, NativeStorage>;

impl RollupBlueprint<Native> for MockSvmRollup<Native>
where
...
impl RollupBlueprint<WitnessGeneration> for MockSvmRollup<WitnessGeneration>
where
...
#[async_trait]
impl FullNodeBlueprint<Native> for MockSvmRollup<Native> {
...
impl WalletBlueprint<Native> for MockSvmRollup<Native> {}
